...
### 09.2 — EAP and IKE_AUTH

| Legacy symbol or behavior | Current mapping | Status | Evidence |
| --- | --- | --- | --- |
| Authentication configuration surface | Original `SIM`, `IMSI`, `IPStackType`, MAC/device/identity mode fields plus current aliases in `Config` | `equivalent-proven` | Redress locks the original field names; production prefers the current AKA provider only when supplied, otherwise uses the original `SIM`, and the original IP-stack field controls CP requests |
| `requiredConfiguredIMSI` and NAI identity selection | Same symbol plus `currentIKEIdentity`, `currentEAPIdentity`, and `currentEAPIdentityForKeyDerivation` | `exact` | IMSI trimming/errors, cached IKE identity, explicit EAP identity state, fast-reauth identity, and permanent-NAI key-derivation fallback follow the recovered branches |
| AKA challenge-mode helpers | `normalizeAKAChallengeMode`, `resolveATCheckcodeValue`, `appendAKAChallengeMetaAttrs`, and `calcAKACheckcodeWithPending` | `equivalent-proven` | The recovered minimal/off/checkcode/recompute aliases, SHA-1/SHA-256 transcript digest, result indication, echo, recomputation, and omission modes are regression-tested and used while building real responses |
| Configuration and traffic selectors | `buildCPRequestPayload` and `buildTrafficSelectorsForIPStack` | `exact` | IPv4 requests 1/3/20; IPv6 requests 8-with-prefix-64/10/21/16390; dual stack preserves that order and TSi/TSr cover both address families |
| Initial IKE_AUTH construction | `buildIKEAuthInitPayloads` | `exact` | Production sends IDi, IDr, CP, ordered ESP SA, TSi, TSr, EAP-only, MOBIKE, ticket request, and initial contact in recovered order; the CHILD_SA SPI and every encoding/config error remain real |
| DEVICE_IDENTITY behavior | `spoofAppleIMEI`, `encodeIMEITBCD`, and `deviceIdentityPayloads` | `equivalent-proven` | Restores fixed TAC `35898336`, IMSI suffix, Luhn digit, TBCD, and paired 3GPP/generic notifications; malformed explicit IMEIs fail instead of producing an invalid wire value |
| Original `handleEAP` ownership contract | `handleEAP` with `applyEAPHandlingResult` as transport owner | `exact` | The handler returns payloads without sending; the production IKE_AUTH state machine sends them exactly once, while the current direct-send adapter remains source-compatible |
| EAP Identity and AKA/AKA′ Identity | `handleRFCEAPRequest`, `buildAKAIdentityResponse`, and transcript capture | `equivalent-proven` | Generic Identity, ANY/PERMANENT/FULLAUTH requests, empty identity responses, fast reauth preference, and retransmission-deduplicated identity transcript are covered by packet-level tests |
| AKA/AKA′ Challenge and MAC | `handleRFCChallenge`, `eapaka` key derivation, and `verifyEAPAKAMAC` | `equivalent-proven` | RAND/AUTN reach the configured real SIM provider, request MAC/checkcode are authenticated, RES/KDF/meta/MAC responses are generated from real keys, MSK is retained, and tampering fails |
| Explicit diagnostic MAC bypass | `DisableEAPMACValidation` and `BuildChallengeResponseWithoutMACValidation` | `exact` | The recovered opt-in skips only request AT_MAC validation, remains disabled by default, emits a warning when used, and still performs real SIM calculation, key derivation, response signing, and self-check |
| Signed-response and validation helpers | `buildSignedEAPResponse`, `buildEAPMACSelfCheckProof`, `eapAttrDigest`, and `validateKnownSimakaAttributes` | `equivalent-proven` | SHA-1/SHA-256 MAC placement is self-verified, unknown non-skippable attributes and malformed lengths fail, and diagnostics hash sensitive material rather than logging raw keys |
| Notifications, synchronization failure, and fast reauthentication | Restored notification path, `buildEAPSyncFailure`, and `verifyEAPReauthMAC` | `equivalent-proven` | Authenticated result indication gates EAP Success; AUTS uses the negotiated method; reauthentication verifies method-specific MAC, counter state, encrypted identity updates, and replaces MSK on the production path |
| Initial IKE key generation | `GenerateIKESAKeys` and shared erasure-enabled derivation | `equivalent-proven` | IKE_SA_INIT invokes the recovered symbol; nonce handling, SKEYSEED, PRF+, key lengths, AEAD/integrity layouts, and failures are vector-tested |
| Final initiator and responder AUTH | `buildIKEAuthFinalPayloads`, `verifyResponderAuth`, and `logResponderAuthMismatch` | `equivalent-proven` | SignedOctets bind the stored IKE_SA_INIT transcript and ID payload; initial certificate AUTH and final MSK AUTH are mandatory in their negotiated modes, constant-time compared, and mismatch logs expose only digests |
| Final IKE_AUTH response | Original byte-taking `handleIKEAuthFinalResp` plus structured helper | `equivalent-proven` | The production loop encodes the received packet into the recovered byte contract, then verifies/decrypts AUTH, rejection notifications, CP, SA and TS narrowing before installing assigned addresses and negotiated CHILD_SA state |
